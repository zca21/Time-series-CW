...
tsdiag(fit_ts8)
```
Model diagnostic plots for the ARIMA(1,1,0) model show no clear patterns in the residuals, no autocorrelation between the residuals and p-values for Ljung-Box statistic consistently high (non-significant) thus there is no evidence of a time structure/s not captured by the model that are predsent in the data. I also note that $\sigma=0.97$ thus is close to one one of the expected values of 1.
...
ks.test(x,y)
```
Performing a Kolmogorov-Smirnov test on the second order differenced time series gives a non significant p-value of 0.184 giving further evidence the transformed time series data is stationary. Thus as all evidence supports that taking the second order difference of the time series data produces a stationary time series. Therefore as this is the minimum number of differences that is producing a stationary time series to avoid potential overdifferencing I assume the time series is a ARIMA d=2 process and now proceed to analyse the acf and pacf correolgrams of the time series after applying a second order difference.
