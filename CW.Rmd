...
```
Performing a Kolmogorov–Smirnov test on the first and last half of the time series data gives a very high p-value of 0.997 thus no trend or heteroscedasticity in the time series.
...
To remove the trend and seasonal component take the difference at lag 48 to remove the periodicity and then difference at lag 1 to remove the trend. Then I analyse this transformed time series.

```{r}
#transforming time series
t_ts5<-diff(diff(tser05),lag=48)

#correlograms of transformed ts
par(mfrow=c(1,2))
acf(t_ts5)
pacf(t_ts5)
```
Both the acf and partial acf correlograms appear to exhibit sinusoidal dampening therefore start by fitting a ARIMA(1,0,1) model to the transformed time series data.

```{r}
#model fitting for parameter estimation
fit_ts5<- arima(t_ts5,order=c(1,0,1))

print(fit_ts5)
tsdiag(fit_ts5)
```
Diagnostic plots shows that there is evidence of time structure not explained by the model as evidence of autocorrelation between residuals and significant p-values for Ljung-Box statistic  meaning we reject the null hypothesis that the autocorrelation function is zero at lags 2 and greater.

Therefore I revise the model first inceasing q to be 2.

```{r}
fit_ts5_2<-arima(t_ts5,order=c(1,0,2))

print(fit_ts5_2)
tsdiag(fit_ts5_2)
```

The diagnostic plots for the ARIMA(1,0,2) model fitted to the transformed time series look much better with no pattern apparent in the residuals and no evidence of autocorrelation between residuals. The Ljung-Box p-values are all also consistently high.

```{r}
#CI for parameters

#CI for alpha 1
se_1<- sqrt(fit_ts5_2$var.coef[1,1])
print(c(fit_ts5_2$coef[1]-2*se_1,fit_ts5_2$coef[1]+2*se_1))
#alpha 1 significantly different from 0

#CI for beta 1
se_2<- sqrt(fit_ts5_2$var.coef[2,2])
print(c(fit_ts5_2$coef[2]-2*se_2,fit_ts5_2$coef[2]+2*se_2))
#beta 1 significantly different from 0

#CI for beta 2
se_3<- sqrt(fit_ts5_2$var.coef[3,3])
print(c(fit_ts5_2$coef[3]-2*se_3,fit_ts5_2$coef[3]+2*se_3))
#beta 2 significantly different from 0

```
Calculating the 95% confidence intervals for the 3 parameters being estimated gave the intervals above. All parameters are significant as all don't include 0.

I now calculate higher order p and q models as an extra check that the ARIMA(1,0,2) process is the best fit for the transformed time series.

```{r}
#p=2, q=2
fit_ts5_3<-arima(t_ts5,order=c(2,0,2))

#CI for alpha 2
se_2<- sqrt(fit_ts5_3$var.coef[2,2])
print(c(fit_ts5_3$coef[2]-2*se_2,fit_ts5_3$coef[2]+2*se_2))
```
Calculating the $\alpha_2$ 95% confidence interval for a ARIMA(2,0,2) model fitted to the transformed time series data gave an interval that included 0, therefore the $\alpha_2$ term is insignificant.

```{r}
#p=1, q=3
fit_ts5_4<-arima(t_ts5,order=c(1,0,3))

#CI for beta 3
se_4<- sqrt(fit_ts5_3$var.coef[4,4])
print(c(fit_ts5_4$coef[4]-2*se_4,fit_ts5_3$coef[4]+2*se_4))
```
Calculating the $\beta_3$ 95% confidence interval for a ARIMA(1,0,3) model fitted to the transformed time series data gave an interval that included 0, therefore the $\beta_3$ term is insignificant.

```{r}
#p=3, q=2
fit_ts5_5<-arima(t_ts5,order=c(3,0,2))

#CI for alpha 2
se_2<- sqrt(fit_ts5_5$var.coef[2,2])
print(c(fit_ts5_5$coef[2]-2*se_2,fit_ts5_5$coef[2]+2*se_2))

#CI for alpha 3
se_3<- sqrt(fit_ts5_5$var.coef[3,3])
print(c(fit_ts5_5$coef[3]-2*se_3,fit_ts5_5$coef[3]+2*se_3))
```
Fitting a ARIMA(3,0,2) model to the data gives an insignificant $\alpha_3$ parameter as the 95% confidence interval included 0.

Thus as the parameters in higher order models are insignificant, I conclude that the ARIMA(1,0,2) is the best fitting model for the transformed time series. Therefore the original time series is an ARIMA(1,0,2) model with estimated parameters $\alpha_1=-0.21$, $\beta_1=-0.77$ and $\beta_2=0.11$. With a period of 48 and trend component.


```{r}
#proof that model needs to be fitted with d=1 once removed seasonality
fit_test<-arima(diff(tser05,lag=48),order=c(2,1,2))
print(fit_test)


plot(diff(tser05,lag=48))
t<-diff(tser05,lag=48)
...
acf(t)
#either trend or d>0 so need to take the difference, but as no apparent linea trend in the data must be d=1 so remeber for modelling
...
print(arima(diff(diff(tser06),lag=),order=c(1,0,2)))
```
