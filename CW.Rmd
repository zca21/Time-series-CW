...
In this coursework I use several tests in the process of identifying my preffered model for the time series I analyse which I now mention in slighlty more detail about below.
...
Before starting with to anaylse the time series I created some simulated ARIMA processes with $d=1$ and plotted the time series and their ACF correlograms which can be viewed above. We see that as expected these time series exhibit a trend and that the acf correlograms spikes do not decrease/ decrease gradually as lag increases. This is due to these time series being non-stationary as proven in lectures. Therefore throughout the coursework I use gradually decreasing spikes as the lag increases in acf correlgrams to be evidence for $d>0$ ARIMA processes and vice-versa if there is no such pattern in the acf correlogram that the time series isn't a ARIMA process with $d>0$ and therefore if there is no seasonal component in the time series being plottedthis provides evidence it is stationary.
...
plot(arima222)
```
I create some simulated ARIMA processes with $d=2$ and produce there plots which can be viewed above. As these plots all appear to be smooth curves, I note that if time series plots appear to be smooth curves it gives an indication they are ARIMA processes with d equal to 2 in this section therefore they will need to be to be differenced twice to provide a stationary time series.
...
Producing the stl decomposition for each possible frequency of period found by using the formula $nobs=12*nfreq*nyear$. As there are 240 observations there are only 4 possible integer combinations of nfreq and nyear which are (4,5),(10,2),(5,4) and (1,20)) that give more than 1 period. Viewing the above plots the first triplet corresponding to nfreq=1 and nyear=20 appears to fit the data the best as the random component of the stl decomposition appears stationary and the acf correlogram doesn't have a wavelike appearance thus the seasonality has been removed.
...
fit_ts6_2<- arima(diff(tser06,lag=12),order=c(1,2,2))
tsdiag(fit_ts6_2)
```
...
```{r}
#fitting p-=1 q=2 model
fit_ts6_2<- arima(diff(tser06,lag=12),order=c(2,1,3))
tsdiag(fit_ts6_2)
```
