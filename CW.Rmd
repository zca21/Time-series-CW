...
## Tests used in this coursework

In this coursework I use several tests in the process of identifying my preferred model for the time series I analyse which I now mention in more detail about below.

The two sample Kolmogorov–Smirnov test is used to identify if 2 samples of data came from the same distribution by testing the null hypothesis that the 2 samples came from a population with the same distribution. In the context of this coursework I use it to provide evidence that the time series being analysed has no trend component or heteroscasdticy (as if this was present in the time series this test would give a significant p-value). Therefore for time series with no seasonal component this test provides evidence that the time series is stationary if the test gives a non-significant p-value.

THe Ljung-Box statistic tests whether of not the autocorrelation function of a stationary process is zero. With a null hypothesis that $H_0: \rho(k)=0$ thus in the context of checking a models residuals, a non-significant p-value at lag k means I fail to reject the null hypothesis and thus there is no evidence from this test that there is autocorrelation between residuals at lag k. If this is true for all lags then it gives evidence that there is no autocorrelation between residuals and therefore the residuals are white noise giving evidence that the model captures all the time structures present in the data,
...
Before starting to analyse the time series I created some simulated ARIMA processes with $d=1$ and plotted the time series and their ACF correlograms which can be viewed above. We see that as expected these time series exhibit a trend and that the acf correlograms spikes do not decrease/ decrease gradually as lag increases. This is due to these time series being non-stationary as proven in lectures. Therefore throughout the coursework I use gradually decreasing spikes as the lag increases in acf correlgrams to be evidence for $d>0$ ARIMA processes and vice-versa if there is no such pattern in the acf correlogram that the time series isn't a ARIMA process with $d>0$ and therefore if there is no seasonal component in the time series being plotted this provides evidence it is stationary.
...
plot(tser01)
```
Initially viewing the plot of the first time series object: it doesn't appear to have a trend or seasonal component, the variation doesn't appear to change as time increases thus appears the time series object is not heteroscedastic.
...
```
Performing a Kolmogorov–Smirnov test on the first and last half of the time series data gives a very high p-value of 0.997 thus it provides evidence of no trend or heteroscedasticity in the time series giving evidence that the process is stationary.
...
```
The correlogram of the ACF has 1 significant spike at lag 0 before cutting off while none of the spikes are significant in the partial ACF correlogram. This combination leads me to believe the time series is Gaussian white noise (MA(0) process) thus I try fitting an ARIMA(0,0,0) model to the data.
...
print(fit_ts1)
```
The fitted model has a standard deviation of 0.94 close to the expected value of 1 with the difference due to the model fitting process not being able estimate the parameters and $\sigma$ exactly.
...
Initial observation of the second time series object: it doesn't appear to have a trend or seasonal component, the variation doesn't appear to change as time increases, thus it appears the time series object is not heteroscedastic.
...
```
I then calculate the $\beta_1$ and $\beta_2$ 95% confidence intervals which are displayed above. Both parameters are significant as their confidence intervals do not contain.
...
To further investigate this time series I fit higher order ARMA models to the data to examine if they are preferential to the ARMA(1,1) model fitted above. 
...
As the models with higher order p or q terms are not significant I conclude that the most suitable model is an ARIMA(1,0,1) with $\alpha_1$ estimated to be -0.52 and $\beta_1$ to be -0.61.
...
```
Creating confidence intervals for the parameters $\alpha_1$, $\alpha_2$ and $\beta_1$ estimated by the mdoel gives the intervals above. I note that $\alpha_1$ is not significantly different from 0 as its confidence interval includes 0 but the $\beta_1$ and $\alpha_2$ parameters are significantly different from 0 (as their confidence intervals do not include 0).
...
```
Fitting an ARIMA(2,0,2) gave a non-significant $\beta_2$ value (as the 95% confidence interval included 0) which can be seen above. Thus this model reduces to an ARIMA(1,0,2) process.

Therefore, I conclude that my preferred model for this time series is an ARIMA(2,0,1) with $\alpha_1=0$, $\alpha_2=-0.31$ and $\beta_1=0.71$,
...
ks.test(x,y)
```
Splitting the time series data in half and performing a Kolmogorov-Smirnov test gives a large non-significant p-value of 0.723 providing evidence that the time series is not heteroscedastic and does not contain a trend and therefore is stationary.
...
```
Calculating 95% confidence intervals for all 4 parameters in the ARIMA(2,0,2) model seen above, as none include 0 all 4 parameters are significant.
...
Therefore as there is no evidence to suggest that the ARIMA(2,0,2) model does not capture the time structure of the data and as it has the lowest AIC this is my preffered model for this time series. The parameters are estimated to be $\alpha_1=-1.41$, $\alpha_2=-0.71$, $\beta_1=1.13$ and $\beta_2=0.53$.
...
plot(arima222)
```
I create some simulated ARIMA processes with $d=2$ and produce their plots which can be viewed above. As these plots all appear to be smooth curves, I note that if time series plots appear to be smooth curves it gives an indication they are ARIMA processes with d equal to 2 in this section therefore they will need to be to be differenced twice to provide a stationary time series.
...
Therefore, overall my preffered model for this time series is an ARIMA(1,1,2) process with parameters estimated to be $\alpha_1=-0.2$, $\beta_1=-0.77$ and $\beta_2=0.93$ and a seasonal component with frequency equal to 48.
...
#creating second order differenced time series
t_ts6<-diff(diff(tser06))
```

Viewing the plots after taking the first and second order difference it is clear that after taking the first difference that the time series is not stationary and still has a trend but after taking the second order difference the trend appears to of been eliminated. Therefore this backs up my original claim that the time series is an ARIMA process with d=2. I note now that there doesn't appear to be any obvious seasonality in the time series however we are told in the question that seasonality exists in time series 5-7. Thus it is likely the seasonal component has a small amplitude and therefore to identify it I will use stl decomposition.

Therefore I take the second order difference of the time series and analyse this transformed time series to try and identify the seasonal component of this time series.
...
Producing the stl decomposition for each possible frequency of period found by using the formula $nobs=12*nfreq*nyear$. As there are 240 observations there are only 4 possible integer combinations of nfreq and nyear which are (4,5),(10,2),(5,4) and (1,20)) that give more than 1 period. Viewing the above plots the first three which correspond to nfreq=1 and nyear=20 appears to fit the data the best as the random component of the stl decomposition appears stationary and the acf correlogram of the extracted random component doesn't have a wavelike appearance thus the seasonality has been removed. The seasonal component also has a very small amplitude of 1 which is much smaller than that of the size of the random component as I hypothesized.

Therefore I conclude that the time series has a frequency of 12.
...
```
Performing a Kolmogorov-Smirnov test on the transformed time series (differenced at lag 12 to remove seasonality and then differenced at lag 1 twice to remove the trend) gives a non-significant p-value of 0.55 indicating that the time series is stationary. The acf also appears to reduce sinusoidally as although it has a spike at lag 12 this is likely some residual seasonality still present in the transformed time series. Therefore I conclude that the transformed time series is stationary and the original time series is an ARIMA process with d=2 and seasonality of frequency 12.
...
pacf(t_ts6)
```
Both the acf and partial acf correlograms appear to be decaying sinusoidally giving evidence that the transformed time series is an ARMA process thus initially I try to fit a ARIMA(1,2,1) model to the original time series data.

```{r}


fit_ts6<- arima(diff(tser06,lag=12),order=c(1,2,1))
tsdiag(fit_ts6)
```
Fitting a ARIMA(1,2,1) model and looking at the diagnostic plots there is evidence of autocorrelation between the residuals and the p-values for the Ljung-Box statistic are significant for lag 2 and greater. This indicates there is still some time structure in the data that the model isn't accounting for. Therefore I try higher order p and q models.
...
for (p in 1:3){
  for(q in 1:3){
    ...
    model_aic<-append(model_aic_temp,AIC(arima(diff(tser06,lag=12),order=c(p,2,q))))
  }
}
...
order(test)
```
Producing models of all possible p and q combinations with p,q>1 I find that the lowest AIC model is an ARIMA(3,2,3)


```{r}
#fitting p=3 q=3 model
fit_ts6_2<- arima(diff(tser06,lag=12),order=c(3,2,3))
tsdiag(fit_ts6_2)
```

The diagnostic plots for the ARIMA(3,2,3) fitted model show there is no clear pattern to the residuals and although there is evidence of autocorrelation between the residuals it is at lag 12 thus is from some residual seasonality. The Ljung-Box statistics p-values are in the null region for lags 8 and less and the rest are just bordering the null region therefore there is not enough evidence to reject the null hypothesis. Therefore there is no evidence of time structure/s not captured by this model.
...
#CI for beta 1
se_4<- sqrt(fit_ts6_2$var.coef[4,4])
print(c(fit_ts6_2$coef[4]-2*se_4,fit_ts6_2$coef[4]+2*se_4))

#CI for beta 2
se_5<- sqrt(fit_ts6_2$var.coef[5,5])
print(c(fit_ts6_2$coef[5]-2*se_5,fit_ts6_2$coef[5]+2*se_5))

#CI for beta 3
se_6<- sqrt(fit_ts6_2$var.coef[6,6])
print(c(fit_ts6_2$coef[6]-2*se_6,fit_ts6_2$coef[6]+2*se_6))
```
Producing 95% confidence intervals for all 6 parameters, I find that $\beta_1$ and $\beta_2$ are insignificant as their confidence intervals include zero however the rest of the parameters are significant as their confidence intervals do not contain 0. 
```{r}
print(fit_ts6_2)
```
I note that the $\sigma$ is estimated to be 3.96 which is larger than the expected values, however this is due to the differencing I performed to remove the seasonality which will increase the variance in the data

Therefore as there is no evidence there is some time structure/s that the ARIMA(3,2,3) model hasn't captured and the leading order p and q parameters ($\alpha_3$ and $\beta_3$) are both significant I choose this as my preffered model as it has the lowest AIC of all models with p and q greater than 1. Thus my preferred model for this time series is an ARIMA(3,2,3) process with  $\alpha_1=-0.45$, $\alpha_2=0.25$, $\alpha_3=-0.15$, $\beta_1=0$, $\beta_2=0$ and $\beta_3=0.97$ with a seasonal component of frequency 12.
...
Using stl decomposition to remove the seasonality and any trend in the data, looking at the remainder it now appears to no longer be periodic. Therefore I consider the frequency of 48 for one period to be correct. The trend component is also negligible this backs up my original claim that there is no trend in this time series.
...
#ordering values from smallest to largest to identify any better models to investigate in detail
order(test)
AIC(arima(t_ts7,order=c(3,0,1)))
AIC(arima(t_ts7,order=c(2,0,0)))
...
```
Producing some simulations of ARIMA processes with d=3 which I plotted above. I see they also look like smooth curves, thus for time series that appear to be smooth curves it gives an indication they are ARIMA processes with either d=2 or d=3 in this section and therefore will need to be differenced either twice or three times to get a stationary series.

In these final three time series I assume no seasonal component exists in them.
...
Looking at the plot for the eighth time series it appears to be none stationary.
...
Therefore I take the 1st difference and produce a plot of the transformed time series and a correlogram of the acf. The correlogram shows the spikes appears to be exhibiting sinusoidal damping giving evidence that the transformed time series is no longer stationary and viewing the plot of the transformed time series, it does not appear to have any obvious trends.
...
ks.test(x,y)
```
Performing a Kolmogorov-Smirnov test by splitting the transformed time series in half the p-value is non-significant at 0.66 giving further evidence the time series is now stationary, thus I conclude that the transformed time series is stationary and therefore the original time series is an ARIMA process with $d=1$ 
...
Viewing the correlograms of the transformed time series the acf appears to dampen sinusoidally meanwhile the partial ACF cuts off after the 1st lag. Therefore the transformed time series appears to be a ARIMA(1,1,0) process.
...
tsdiag(fit_ts8)
```
Model diagnostic plots for the ARIMA(1,1,0) model show no clear patterns in the residuals, no autocorrelation between the residuals and p-values for Ljung-Box statistic consistently high (non-significant) thus there is no evidence of a time structure/s not captured by the model that are present in the data. I also note that $\sigma=0.97$ thus is close to one one of the expected values of 1.
...
#10 is lowest index which corresponds to ARIMA(1,1,1)
...
Plotting the time series I observe it is a smooth curve thus it is likely to be an ARIMA process with d=2 or 3 so I plot transformed time series taking differences 1, 2 and 3 times.
...
ks.test(x,y)
```
Performing a Kolmogorov-Smirnov test on the second order differenced time series gives a non significant p-value of 0.184 giving further evidence the transformed time series data is stationary. Thus as all evidence supports that taking the second order difference of the time series data produces a stationary time series. Therefore as this is the minimum number of differences that is producing a stationary time series to avoid potential overdifferencing I assume the time series is a ARIMA d=2 process and now proceed to analyse the acf and pacf correlograms of the time series after applying a second order difference.
...
```
The partial acf correlogram of the transformed time series appears to exhibit sinusoidal dampening while the acf correlogram appears to also exhibit sinusoidal damping thus I conclude the stationary time series is an ARMA process.
...
```
Creating 95% confidence intervals for the parameters estimated of the ARIMA(4,2,4) model I find that all parameters are significant as none of the calculated intervals include 0.

```{r}
print(fit_ts9)
```
The estimated $\sigma$ is 2.85 which is close to 3 one of the expected values of $\sigma$.
...
plot(tser10)
```
Plotting the tenth time series it appears it could be non stationary as could be heteroscedastic.
...
Performing a Kolmogorov-Smirnov test gives a non-significant p-value of 0.1124 giving evidence that the time series is stationary and therefore it is an ARIMA process with therefore d=0.
...
pacf(tser10)
```
The acf appears to exhibit sinusoidal decay giving more evidence that the time series is stationary. Due to the Kolmogorov-Smirnov test and acf correlogram shape I conclude that the time series is stationary. The pacf of the time series cuts off after a lag of 2 thus as the acf tails off there is evidence that the time series is an ARIMA(2,0,0).
...
#extracting the AIC's into a vector
test<-unlist(model_aic)
#ordering values from smallest to largest to identify potentail best model
order(test)
#17 corresponds to ARIMA(2,0,0) model
```
